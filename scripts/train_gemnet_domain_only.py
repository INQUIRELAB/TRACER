#!/usr/bin/env python3
"""
Train GemNet with Domain Embedding Only (No FiLM) for Ablation Study
This trains a GemNet model with domain embeddings but without FiLM modulation.
"""
...
def get_domain_id(domain_str: str) -> int:
    """Map domain string to domain ID."""
...
def custom_collate_fn(batch_list):
    """Custom collate function for domain_id."""
...
def sample_to_pyg_data(sample, norm_mean=None, norm_std=None):
    ...
    # Create edges using distance matrix
    cutoff = 10.0
...
def train_domain_only(
    num_epochs: int = 50,
    batch_size: int = 16,
    learning_rate: float = 1e-4,
    device: str = "cuda",
    output_dir: str = "models/gemnet_domain_only"
):
    """Train GemNet with domain embeddings but no FiLM."""
    
    logger.info("=" * 80)
    logger.info("  TRAINING GEMNET DOMAIN EMBEDDING ONLY (ABLATION)")
    logger.info("=" * 80)
    logger.info("")
    logger.info("Configuration:")
    logger.info(f"   Domain embeddings: ENABLED")
    logger.info(f"   FiLM adaptation: DISABLED")
    logger.info("")
    ...
    # Create model (Domain embedding but NO FiLM)
    logger.info("\n🔧 Creating GemNet model (domain embedding, NO FiLM)...")
    ...
    model = GemNetWrapper(
        ...
        use_film=False,   # NO FiLM modulation
        num_domains=5,    # Domain embeddings ENABLED
        film_dim=16       # Domain embedding dimension
    ).to(device)
    
    logger.info(f"   Model parameters: {sum(p.numel() for p in model.parameters()):,}")
    logger.info(f"   ✅ Domain embeddings enabled, FiLM disabled")
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs} - Train"):
            ...
            # Extract domain_id (for domain embeddings, but no FiLM)
            ...
            # Forward pass (domain_id for embeddings, but no FiLM modulation)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train GemNet Domain Embedding Only for Ablation Study")
    parser.add_argument("train", type=str, help="Train command")
    parser.add_argument("--num-epochs", type=int, default=50, help="Number of training epochs")
    parser.add_argument("--batch-size", type=int, default=16, help="Batch size")
    parser.add_argument("--learning-rate", type=float, default=1e-4, help="Learning rate")
    parser.add_argument("--device", type=str, default="cuda", help="Device (cuda/cpu)")
    parser.add_argument("--output-dir", type=str, default="models/gemnet_domain_only", help="Output directory")
